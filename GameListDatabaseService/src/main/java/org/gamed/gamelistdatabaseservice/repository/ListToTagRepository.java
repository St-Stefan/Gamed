...
public interface ListToTagRepository extends JpaRepository<ListToTag, String> {
    ...
    @Query("SELECT ltt FROM ListToTag ltt WHERE ltt.list.id = :listId")
    List<ListToTag> getListToTagsByListId(@Param("listId") String listId);

    @Query("SELECT ltt FROM ListToTag ltt WHERE ltt.tag.id = :tagId")
    List<ListToTag> getListToTagsByTagId(@Param("tagId") String tagId);

    @Query("SELECT ltt FROM ListToTag ltt WHERE ltt.list.id = :listId AND ltt.tag.id = :tagId")
    ...
    @Query("SELECT CASE WHEN COUNT(ltt) > 0 THEN true ELSE false END FROM ListToTag ltt WHERE ltt.list.id = :listId AND ltt.tag.id = :tagId")
